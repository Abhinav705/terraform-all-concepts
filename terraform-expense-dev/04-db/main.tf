...
module "records" {
  ...
  records = [
    {
      name    = "db"#it will append name+zone_name = db.abhinavk.fun
      type    = "CNAME" #CNAME is used to map the another domain since RDS end point is not an IP. it is a domain.
      ttl = 1
      records = [
        module.db.db_instance_address #db_instance_address gives you address of RDS instance
      ]
    }
  ]
}
